...
export default function PublicProfile({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!ready) return <div className="py-16 text-center text-mute">Cargando perfil…</div>;
  if (!user) notFound();
    ...
    <div className="mx-auto max-w-3xl">
      <div className="card mb-6 flex items-center gap-4 p-5">
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-mute">{user.tradesCompleted} trueques · {user.bio}</p>
        </div>
      </div>
      <h2 className="mb-3 font-display text-xl">Ofertas abiertas</h2>
